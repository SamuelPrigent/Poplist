...
import {
  invalidateAllWatchlists,
  invalidateUserProfiles,
  removeAuthUserProfile,
  removeAuthWatchlists,
  removeMe,
...
// Purge (sans refetch) toutes les queries liées à la session : /auth/me
// (CRITIQUE : `removeQueries` et non `invalidateQueries`, sinon refetch → 401 →
// auto-logout → boucle infinie), mes watchlists + détails auth, profil privé.
// Les queries publiques (featured, genres, profils publics) restent en cache :
// elles sont valides quelle que soit la session.
function purgeAuthScopedQueries(queryClient: QueryClient) {
  removeMe(queryClient);
  removeAuthWatchlists(queryClient);
  removeAuthUserProfile(queryClient);
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const handleAutoLogout = useCallback(async () => {
    ...
    purgeAuthScopedQueries(queryClient);
    setStoredAuthState(false, null);
    setOptimisticAuth(false);
  ...
  const login = async (email: string, password: string) => {
    ...
    invalidateAllWatchlists(queryClient);
  };
