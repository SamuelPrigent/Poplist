...
/** Switcher responsive : < 750px → drawer bottom, sinon modale desktop. */
...
>((props, ref) => {
  ...
  return isMobile ? (
    <EditListOfflineDrawerShell {...props} formRef={ref} />
  ) : (
    <EditListOfflineModalShell {...props} formRef={ref} />
  );
});
EditListDialogOffline.displayName = 'EditListDialogOffline';

type ShellProps = EditListDialogOfflineProps & { formRef: Ref<EditListDialogOfflineRef> };

// -------------------------------------------------------------------------
// Shell modale (desktop) — inchangé.
// -------------------------------------------------------------------------
function EditListOfflineModalShell({ open, onOpenChange, onSuccess, watchlist, formRef }: ShellProps) {
...
// Shell drawer (mobile)
// -------------------------------------------------------------------------
function EditListOfflineDrawerShell({ open, onOpenChange, onSuccess, watchlist, formRef }: ShellProps) {
  const { content } = useLanguageStore();
  return (
...
// -------------------------------------------------------------------------
// Corps de formulaire partagé.
// -------------------------------------------------------------------------
const EditListOfflineForm = forwardRef<
  EditListDialogOfflineRef,
  {
    open: boolean;
    onClose: () => void;
...
>(({ open, onClose, onSuccess, watchlist }, ref) => {
  const { content } = useLanguageStore();
...
EditListOfflineForm.displayName = 'EditListOfflineForm';
