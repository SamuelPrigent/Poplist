...
export function AddCollaboratorPopover({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const response = await fetch(`/api/auth/username/check/${encodeURIComponent(username)}`);
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : content.watchlists.collaborators?.addError || "Échec de l'ajout du collaborateur"
      );
    } finally {
      setIsAdding(false);
    }
