 ...
 */
function WatchProviderBubbleInner({ providerName, logoPath, index = 0 }: WatchProviderBubbleProps) {
      ...
      <TooltipTrigger asChild>
        ...
        >
          <Image
            ...
            sizes="36px"
            className={
              providerLogo ? `object-contain ${providerLogo.className ?? 'p-1'}` : 'object-cover'
            }
...
function normalizeProviderName(name: string): string {
  ...
  if (normalized.includes('hbo') || normalized === 'max' || normalized.includes('max amazon'))
    return 'hbo';
...
export function WatchProviderList({ providers, maxVisible = 5 }: WatchProviderListProps) {
