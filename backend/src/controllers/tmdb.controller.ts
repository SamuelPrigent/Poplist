...
export const getTrending = async (c: C) => {
  ...
  try {
    const data = (await tmdb.getTrending(
      ...
      c.req.query('page') || '1',
    )) as z.infer<typeof tmdbListResponseSchema>;
    return c.json(data satisfies z.infer<typeof tmdbListResponseSchema>);
  } catch (error) {
...
export const discover = async (c: C) => {
  ...
  try {
    ...
    const data = (await tmdb.discover(type, fetchParams)) as z.infer<typeof tmdbListResponseSchema>;
    return c.json(data satisfies z.infer<typeof tmdbListResponseSchema>);
  } catch (error) {
...
export const getGenres = async (c: C) => {
  ...
  try {
    const data = (await tmdb.getGenres(type, c.req.query('language') || 'fr-FR')) as z.infer<
      typeof tmdbGenresResponseSchema
    >;
    return c.json(data satisfies z.infer<typeof tmdbGenresResponseSchema>);
  } catch (error) {
...
export const getProviders = async (c: C) => {
  ...
  try {
    const data = (await tmdb.getProviders(type, id)) as z.infer<typeof tmdbProvidersResponseSchema>;
    return c.json(data satisfies z.infer<typeof tmdbProvidersResponseSchema>);
  } catch (error) {
