...
export async function getWatchProviders(
	...
	try {
		...
		for (const provider of allProviders) {
			if (
				isAllowedProvider(provider.name) &&
				!uniqueProviders.has(provider.name)
			) {
...
export async function searchMedia(
	...
	try {
		...
		const finalResults = [...enrichedResults, ...filteredResults.slice(15)];
...
export async function getMovieFullDetails(
	...
	try {
		...
		const cast =
			movieData.credits?.cast?.slice(0, 3).map((member) => ({
				name: member.name,
...
export async function getTVFullDetails(
	...
	try {
		...
		// Extract top 3 cast members
		const cast =
			tvData.credits?.cast?.slice(0, 3).map((member) => ({
				name: member.name,
				character: member.character,
				profileUrl: buildProfileUrl(member.profile_path),
			})) || [];
